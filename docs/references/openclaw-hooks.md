---
title: OpenClaw Hooks 接入说明
agent: OpenClaw
source_primary: https://docs.openclaw.ai/automation/hooks
source_secondary: https://docs.openclaw.ai/plugins/hooks
fetched_at: 2026-08-12
notes: 包含 Internal Gateway Hooks 与 Plugin Hooks 两套机制
---
...
Hooks can also be bundled inside plugins. `openclaw hooks list` shows both standalone hooks and plugin-managed hooks (displayed as `plugin: `).
...
(for example `command:nwe`), and `openclaw hooks info ` flags them, so a
...
4. **Workspace hooks**: ` /hooks/` (per-agent, disabled by default until explicitly enabled)
...
| --------------------- | ---------------------------------------------------- | -------------------------------------------------------------- |
| session-memory | `command:new`, `command:reset`, `session:auto-reset` | Saves session context to ` /memory/` |
...
`plugin: `. Use those for side effects and compatibility with hook packs, not
...
`hooks.timeouts. ` overrides `hooks.timeoutMs`, which overrides the
...
`--raw-stream-path ` to write raw model stream events to a jsonl file.
...
`plugins.entries..hooks.allowPromptInjection`. Prompt-mutating hooks and
