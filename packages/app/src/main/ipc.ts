...
export function registerIpc(sentinel: Sentinel, windows: WindowManager): void {
  ...
  handle(IPC.sendTaskMessage, (taskId: string, text: string) =>
    sentinel.sessionChat.send(taskId, text)
  )

  handle(IPC.getConversation, (taskId: string) => sentinel.sessionChat.conversation(taskId))
