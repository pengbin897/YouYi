...
import { SessionChat } from './router/session-chat.js'
...
export class Sentinel extends EventEmitter {
  ...
  /** 前端会话续聊（v1.1）：Dashboard 直发消息进选中的 Workbuddy 会话 */
  readonly sessionChat: SessionChat
  ...
  constructor() {
    ...
    this.sessionChat = new SessionChat({
      settings: this.settings,
      store: this.store,
      relay: this.relay,
      adapters: this.adapters,
  ...
  async stop(): Promise<void> {
    ...
    this.digest.stop()
    // 先叫停仍在跑的无头续跑，避免留下孤儿子进程
    this.sessionChat.dispose()
