...
export class SqliteStore implements Store {
  ...
  readonly tasks: TaskRepo = {
    ...
    listBySession: (agentId: AgentId, sessionId: string): Task[] => {
      const rows = this.db
        .prepare(
          `SELECT * FROM tasks WHERE agent_id = ? AND session_id = ? ORDER BY started_at ASC`
        )
        .all(agentId, sessionId) as TaskRow[]
      return rows.map(rowToTask)
    },
