...
function TaskDetail({ taskId, onClose }: { taskId: string; onClose: () => void }): ReactElement {
  ...
  const agentId = data?.task.agent_id
  const sessionId = data?.task.session_id

  // 会话对话流：首次加载 + 该会话有新事件到达时刷新（新一轮任务/回复都走事件管线）
  useEffect(() => {
    if (!sessionId || !agentId) return
    let disposed = false
    const load = (): void => {
      void window.youyi.getConversation(taskId).then((entries) => {
        if (!disposed) setChat(entries)
      })
    }
    load()
    const off = window.youyi.on<UnifiedEvent>(IPC.pushEvent, (event) => {
      if (event.agent_id === agentId && event.task_meta.session_id === sessionId) load()
    })
    return () => {
      disposed = true
      off()
  ...
  // v1.1：续聊仅支持 Workbuddy 的已有会话，其余如实说明，不开放入口
  const canChat = agentId === 'workbuddy' && Boolean(sessionId)
  ...
  const send = async (): Promise<void> => {
    ...
    setSending(true)
    setFeedback(null)
    const result = await window.youyi.sendTaskMessage(taskId, text)
    if (result.ok) {
      setInput('')
      setFeedback(
        result.via === 'stop-hook'
          ? '已排队，Workbuddy 这一轮结束后就会收到。'
          : '已发送，Workbuddy 开始处理这一轮。'
      )
    } else {
      ...
      <aside className="drawer" onClick={(e) => e.stopPropagation()}>
        {!data ? (
          ...
          <>
            ...
            <div className="section__title">对话</div>
            <div className="chat">
              {chat === null ? (
                <div className="empty">加载中…</div>
              ) : chat.length === 0 ? (
                <div className="empty">这个会话还没有记录。</div>
              ) : (
                chat.map((entry) => (
                  <div key={entry.id} className={`chat__entry chat__entry--${entry.role}`}>
                    <div className="chat__text">{entry.text}</div>
                    <div className="chat__time">
                      {entry.role === 'user' ? '我' : agentName(data.task.agent_id)} ·{' '}
            ...
            {canChat ? (
              <div className="chat-input">
                ...
                />
                <button className="btn btn--primary btn--sm" disabled={sending || !input.trim()} onClick={() => void send()}>
                  {sending ? '发送中…' : '发送'}
                </button>
              </div>
            ) : (
              <div className="notice" style={{ marginTop: 10 }}>
                续聊目前仅支持 Workbuddy 的已有会话。
              </div>
            )}
            {feedback && <div className="chat-feedback">{feedback}</div>}

            <div className="section__title" style={{ marginTop: 18 }}>
              事件时间线（{data.events.length}）
            </div>
